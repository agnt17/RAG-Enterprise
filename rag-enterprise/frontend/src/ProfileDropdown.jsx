...
import { Settings, HelpCircle, PlusCircle, LogOut, Sun, Moon, Monitor, ChevronRight, Check } from "lucide-react"
          ...
          >
            ...
            <div className="py-2">
              ...
              <motion.button
                ...
                className={`${itemCls} ${showThemeMenu ? (isDark ? "bg-white/[0.08]" : "bg-slate-50") : ""}`}
              >
                <Sun size={17} strokeWidth={1.5} />
                <span className="flex-1 text-left">Theme</span>
                <span className={`text-xs mr-1 ${isDark ? "text-[#48484a]" : "text-[#86868b]"}`}>
                  {themeOptions.find(t => t.key === themeMode)?.label}
                </span>
          ...
          <motion.div
            ...
            className={`fixed rounded-xl shadow-xl border py-1 z-[9999]
              ${isDark
                ? "bg-black/80 backdrop-blur-2xl border-white/[0.1]"
                : "bg-white/90 backdrop-blur-2xl border-slate-200/50"}`}
            style={{ top: flyoutPos.top, left: flyoutPos.left, minWidth: 140 }}
          >
            {themeOptions.map(({ key, label, icon }) => (
              <motion.button
                ...
                className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm transition-colors duration-100 cursor-pointer
                  ${themeMode === key
                    ? isDark ? "bg-[#0a84ff]/12 text-[#0a84ff]" : "bg-blue-50 text-[#0071e3]"
                    : isDark ? "hover:bg-white/[0.08] text-[#f5f5f7]/80" : "hover:bg-slate-50 text-slate-700"
                  }`}
              >
                {icon}
                <span className="flex-1 text-left">{label}</span>
                {themeMode === key && <Check size={14} />}
              </motion.button>
            ))}
          </motion.div>,
          document.body
