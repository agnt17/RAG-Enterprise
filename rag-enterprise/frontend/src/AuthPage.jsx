...
export default function AuthPage({ onLogin, resolvedTheme = "dark" }) {
  ...
  // Send Google's ID token to our backend for verification
    ...
    const detail = err?.response?.data?.detail
    if (typeof detail === "object" && detail?.code === "ACCOUNT_NOT_FOUND") {
      setMode("register")
    ...
    return (
      <div className="relative min-h-screen overflow-x-hidden px-3 py-4 sm:px-5 sm:py-6 lg:px-6 lg:py-0">
        ...
        />
        <div
          className={`pointer-events-none absolute inset-0 opacity-40 ${isDark
        ...
        >
          <motion.aside
            ...
            className={`${paneHeightCls} relative hidden flex-col overflow-hidden rounded-[30px] border p-4 sm:p-6 lg:flex lg:p-8 ${isDark
          ...
          <motion.section variants={staggerItem} className={`${paneHeightCls} relative mx-auto w-full max-w-xl lg:max-w-none`}>
            ...
            <motion.div variants={staggerItem} className={cardCls}>
              <div
                className={`pointer-events-none absolute inset-x-0 top-0 h-px ${isDark ? "bg-linear-to-r from-transparent via-cyan-300/70 to-transparent" : "bg-linear-to-r from-transparent via-sky-500/70 to-transparent"
                  }`}
              />
